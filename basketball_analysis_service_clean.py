...
# --- Service Initialization (on service startup) - OPTIMIZED FOR PERFORMANCE ---
mp_pose = mp.solutions.pose
pose_model = mp_pose.Pose(
    static_image_mode=False, 
    model_complexity=0,  # OPTIMIZED: Use simplest model for better performance
    enable_segmentation=False,  # OPTIMIZED: Disable segmentation to save memory
    min_detection_confidence=0.7,  # OPTIMIZED: Slightly higher for better accuracy
...
def process_video_for_analysis(job: VideoAnalysisJob, ideal_shot_data):
    ...
    try:
        ...
        # Limit processing for deployment efficiency - OPTIMIZED FOR MEMORY
        max_frames = min(total_frames, 100)  # Process max 100 frames (3-5 seconds at 30fps) - REDUCED for stability
    ...
    # Process frames with timeout protection - OPTIMIZED
    import time
    start_time = time.time()
    max_processing_time = 90  # 90 seconds max processing time - REDUCED for faster feedback
